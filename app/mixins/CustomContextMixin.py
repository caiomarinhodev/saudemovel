...
class DashboardMixin(ContextMixin):
    def get_context_data(self, **kwargs):
        ...
        kwargs['bd_space'] = float(float(float(len(motoristas)+len(pedidos)+len(users)+len(notificacoes)+len(locations)+len(bairros)+len(estabelecimentos)+len(pontos))/10000)*100)
